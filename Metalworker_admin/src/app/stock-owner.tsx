  ...
  const filteredStocks = useMemo(() => {
    ...
    return stocks.filter(
      (s) =>
        ...
        (s.tn_no ?? "").toLowerCase().includes(q)
    );
  }, [stocks, search]);

  async function handleSave(input: OwnerStockInput) {
    if (editingItem) {
      const res = await updateOwnerStock(editingItem.id, input);
      if (!res.ok) throw new Error(res.error || "Failed to update.");
    } else {
      const res = await createOwnerStock(input);
      if (!res.ok) throw new Error(res.error || "Failed to create.");
    }
    await loadStocks();
  }

  function handleEdit(item: OwnerStock) {
    setEditingItem(item);
    setShowForm(true);
  }
        ...
        {loading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>
              {search ? "No Results" : "No Owner Stock Records"}
            </Text>
            <Text style={styles.emptyText}>
              {search
                ? "Try a different search term."
                : "Add your first owner stock record above."}
            </Text>
          </View>
        ) : (
            ...
            {filteredStocks.map((item) => (
              <View key={item.id} style={styles.stockRow}>
                <View style={styles.stockRowMain}>
                  {item.drawing_photo_url ? (
                    <Image
                      source={{ uri: item.drawing_photo_url }}
                  ...
                  <View style={styles.stockInfo}>
                    <Text style={styles.stockTitle} numberOfLines={1}>
                      {item.folder_no
                        ...
                        : item.source_of_metal || "Untitled"}
                    </Text>
                    <Text style={styles.stockMeta} numberOfLines={1}>
                      {item.metal_type || "—"} • {item.source_of_metal || "—"}
                    </Text>
                    {item.amount_purchase != null && (
                      <Text style={styles.stockAmount}>
                        ₹{item.amount_purchase.toLocaleString()}
                      </Text>
                    )}
