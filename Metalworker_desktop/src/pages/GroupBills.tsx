...
import {
  ...
  fetchBillGroupPhotos, addBillGroupPhoto, removeBillGroupPhoto,
...
export default function GroupBillsPage() {
  ...
  const [photosGroup, setPhotosGroup] = useState<BillGroup | null>(null);
  const [photos, setPhotos] = useState<BillGroupPhoto[]>([]);
  const [photosLoading, setPhotosLoading] = useState(false);
  const [photoFile, setPhotoFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);

  const load = useCallback(async () => {
    try { const list = await fetchBillGroups(); setGroups(list); } catch { } finally { setLoading(false); }
  }, []);
  ...
  function openAdd() {
    setEditingGroup(null); setFormName(""); setFormDate(new Date().toISOString().split("T")[0]);
    setFormMessage(null); setShowFormModal(true);
  }

  function openEdit(group: BillGroup) {
    setEditingGroup(group); setFormName(group.name); setFormDate(group.group_date);
    setFormMessage(null); setShowFormModal(true);
  }

  async function openPhotos(group: BillGroup) {
    setPhotosGroup(group); setPhotos([]); setPhotosLoading(true);
    setPhotoFile(null); setShowPhotosModal(true);
    try { const list = await fetchBillGroupPhotos(group.id); setPhotos(list); }
  ...
  async function handleSave() {
    ...
    setFormLoading(true);
    try {
      const input: BillGroupInput = { name: formName.trim(), group_date: formDate };
      if (editingGroup) {
        const res = await updateBillGroup(editingGroup.id, input);
  ...
  async function handleDelete(group: BillGroup) {
    if (!window.confirm(`Delete bill group "${group.name}"? This will also delete all photos in this group.`)) return;
    try { const res = await deleteBillGroup(group.id); if (res.ok) await load(); else window.alert(res.error || "Failed."); }
  ...
  async function handleUploadPhoto() {
    ...
    try {
      ...
      const res = await addBillGroupPhoto(photosGroup.id, result.secureUrl, result.publicId);
      if (res.ok && res.data) { setPhotos((prev) => [...prev, res.data!]); setPhotoFile(null); }
      else window.alert(res.error || "Failed to add photo.");
  ...
  async function handleRemovePhoto(photoId: string) {
    ...
    try {
      const res = await removeBillGroupPhoto(photoId);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-warning-muted flex items-center justify-center"><FileText size={20} className="text-warning" /></div>
          <div><h1 className="text-xl font-bold text-text">Group Bills</h1><p className="text-sm text-text-muted">{filtered.length} group{filtered.length !== 1 ? "s" : ""}</p></div>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search bill groups..."
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            {filtered.length === 0 ? (
              <tr><td colSpan={4} className="px-5 py-12 text-center"><AlertTriangle size={32} className="mx-auto text-text-muted/30 mb-2" /><p className="text-sm text-text-muted">{search ? "No groups match." : "No bill groups yet."}</p></td></tr>
            ) : filtered.map((group) => (
              <tr key={group.id} className="hover:bg-surface-hover/50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => openPhotos(group)} className="p-2 rounded-lg text-text-muted hover:text-primary hover:bg-primary-muted transition-all cursor-pointer" title="Photos"><Image size={16} /></button>
      ...
      {/* Photos Modal */}
      <Modal open={showPhotosModal} onClose={() => setShowPhotosModal(false)} title={`Photos — ${photosGroup?.name ?? ""}`}>
        <div className="space-y-4">
          {photosLoading ? (
            <div className="flex justify-center py-8"><Loader2 size={24} className="text-primary animate-spin" /></div>
          ) : photos.length === 0 ? (
            <div className="text-center py-8"><Image size={32} className="mx-auto text-text-muted/30 mb-2" /><p className="text-sm text-text-muted">No photos yet.</p></div>
          ) : (
