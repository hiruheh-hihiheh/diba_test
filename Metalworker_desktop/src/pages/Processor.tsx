...
export default function ProcessorPage() {
  ...
  const processorUsers = useMemo(() => {
    let result = workers.filter((w) => w.role === "processor");
  ...
  async function handleCreate() {
    ...
    if (!/^[a-z0-9._-]{3,30}$/.test(cleanUsername)) {
      setAddMessage({ type: "error", text: "Username must be 3-30 characters (letters, numbers, dots, underscores, hyphens)." });
      return;
    }
    ...
    if (!cleanUsername.endsWith("_processor")) {
      setAddMessage({ type: "error", text: "Processor usernames must end with _processor." });
      return;
    }

    try {
      setAddLoading(true);
      const result = await createWorkerUser(cleanUsername, addPassword, "processor");
      if (!result.ok) {
  ...
  async function handleEditSave() {
    ...
    try {
      ...
      if (cleanUsername !== editingWorker.username.toLowerCase()) {
        if (!cleanUsername.endsWith("_processor")) {
          setEditMessage({ type: "error", text: "Processor usernames must end with _processor." });
          setEditLoading(false);
          return;
        }
      ...
      await loadWorkers();
      setTimeout(() => { setShowEditModal(false); setEditMessage(null); }, 1000);
    } catch (err) {
  ...
  async function handleDelete(worker: Profile) {
    ...
    try {
      ...
      if (res.ok) { await loadWorkers(); } else { window.alert(res.error || "Failed to delete user."); }
    } catch (err) {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[22px] font-extrabold text-text">Processor Management</h1>
            <p className="text-[14px] text-text-muted mt-0.5">{processorUsers.length} processor user{processorUsers.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} disabled={refreshing} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface border border-border text-[14px] text-text-muted hover:text-text hover:bg-surface-hover transition-all cursor-pointer">
            <RefreshCw size={16} className={refreshing ? "animate-spin" : ""} />
          </button>
          <button
            onClick={() => { setAddMessage(null); setAddUsername(""); setAddPassword(""); setShowAddModal(true); }}
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-lg">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search processors..."
            ...
            {processorUsers.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-16 text-center">
                  ...
                  <p className="text-[15px] text-text-muted">{search || filter !== "all" ? "No processors match your search or filter." : "No processor users yet."}</p>
                </td>
              </tr>
            ) : (
              processorUsers.map((worker) => (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        {worker.full_name && <p className="text-[13px] text-text-muted mt-0.5">{worker.full_name}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-2 text-[13px] font-bold px-3 py-1.5 rounded-full ${
                      worker.is_active ? "bg-success-muted text-success" : "bg-danger-muted text-danger"
                    }`}>
                      {worker.is_active ? <UserCheck size={14} /> : <UserX size={14} />}
                      {worker.is_active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="px-6 py-4 hidden lg:table-cell"><p className="text-[14px] text-text-muted">{formatDate(worker.created_at)}</p></td>
                  <td className="px-6 py-4 hidden lg:table-cell"><p className="text-[14px] text-text-muted">{formatLastLogin(worker.last_login_at)}</p></td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(worker)} className="p-2.5 rounded-lg text-text-muted hover:text-primary hover:bg-primary-muted transition-all cursor-pointer" title="Edit"><Edit3 size={18} /></button>
                      <button onClick={() => handleDelete(worker)} className="p-2.5 rounded-lg text-text-muted hover:text-danger hover:bg-danger-muted transition-all cursor-pointer" title="Delete"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <Modal open={showAddModal} onClose={() => setShowAddModal(false)} title="Add Processor User">
        <div className="space-y-5">
          <div>
            ...
            <input type="text" value={addUsername} onChange={(e) => { setAddUsername(e.target.value); setAddMessage(null); }} placeholder="e.g. john_processor" autoCapitalize="none"
            ...
            <p className="text-xs text-text-muted mt-1.5">Must end with <code className="text-primary">_processor</code></p>
          </div>
          <div>
            <label className="block text-[12px] font-bold text-text-muted uppercase tracking-[0.1em] mb-2.5">Password</label>
            <div className="relative">
              <input type={showAddPassword ? "text" : "password"} value={addPassword} onChange={(e) => { setAddPassword(e.target.value); setAddMessage(null); }} placeholder="Min 6 characters"
              ...
              <button type="button" onClick={() => setShowAddPassword(!showAddPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text transition-colors cursor-pointer p-1">
                {showAddPassword ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
          </div>
          {addMessage && (
            <div className={`px-5 py-4 rounded-xl text-[14px] animate-scale-in ${addMessage.type === "error" ? "bg-danger-muted border border-danger/20 text-danger" : "bg-success-muted border border-success/20 text-success"}`}>
              {addMessage.text}
            </div>
          )}
      ...
      <Modal open={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Processor User">
        <div className="space-y-5">
          <div>
            ...
            <input type="text" value={editFullName} onChange={(e) => setEditFullName(e.target.value)} placeholder="Enter full name"
              className="w-full px-5 py-3.5 rounded-xl bg-bg border border-border text-[15px] text-text placeholder:text-text-muted/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" />
          </div>
          <div>
            <label className="block text-[12px] font-bold text-text-muted uppercase tracking-[0.1em] mb-2.5">Username</label>
            <input type="text" value={editUsername} onChange={(e) => setEditUsername(e.target.value)} autoCapitalize="none"
              className="w-full px-5 py-3.5 rounded-xl bg-bg border border-border text-[15px] text-text placeholder:text-text-muted/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" />
            <p className="text-xs text-text-muted mt-1.5">Must end with <code className="text-primary">_processor</code></p>
          </div>
          <div>
            <label className="block text-[12px] font-bold text-text-muted uppercase tracking-[0.1em] mb-2.5">Status</label>
            <div className="flex items-center gap-2">
              <button onClick={() => setEditIsActive(true)} className={`flex-1 py-3 rounded-xl text-[14px] font-bold transition-all cursor-pointer border ${editIsActive ? "bg-success-muted border-success/30 text-success" : "bg-surface border-border text-text-muted hover:bg-surface-hover"}`}>Active</button>
              <button onClick={() => setEditIsActive(false)} className={`flex-1 py-3 rounded-xl text-[14px] font-bold transition-all cursor-pointer border ${!editIsActive ? "bg-danger-muted border-danger/30 text-danger" : "bg-surface border-border text-text-muted hover:bg-surface-hover"}`}>Inactive</button>
            </div>
          </div>
          {editMessage && (
            <div className={`px-5 py-4 rounded-xl text-[14px] animate-scale-in ${editMessage.type === "error" ? "bg-danger-muted border border-danger/20 text-danger" : "bg-success-muted border border-success/20 text-success"}`}>
              {editMessage.text}
            </div>
          )}
          <div className="flex justify-end gap-2 pt-2">
